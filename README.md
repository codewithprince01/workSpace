...
- **Backend**: Built using Express.js with MongoDB as the database.

## Requirements

- Node.js version v18 or newer
- MongoDB version v7 or newer
...
# Start services (Express mode - includes MongoDB, Redis, MinIO)
...
```bash
# Start MongoDB and point backend to it via MONGO_URI
cd backend
# Example:
...
- **Express Mode** (default): All services bundled (MongoDB, Redis, MinIO)
  ```bash
  docker compose --profile express up -d
  ```

- **Advanced Mode**: Use external services (AWS S3, Azure Blob, external MongoDB)
  ```bash
...
- **Database**: MongoDB credentials and connection settings
