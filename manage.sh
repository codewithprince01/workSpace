...
view_logs() {
    ...
    case $log_choice in
        ...
        4) $compose_cmd logs -f --tail=100 mongo ;;
...
backup_data() {
    ...
    # Backup database
    print_info "[1/4] Backing up MongoDB database..."
    $compose_cmd exec -T mongo mongodump \
        --username "${DB_USER:-worklenz}" \
        --password "${DB_PASSWORD}" \
        --authenticationDatabase admin \
        --db "${DB_NAME:-worklenz_db}" \
        --archive > "$backup_dir/database.archive"
...
restore_data() {
    ...
    print_warning "⚠️  Current MongoDB, Redis, and MinIO data will be LOST!"
    echo ""
    ...
    if [ -f "$temp_dir/$backup_content/database.archive" ]; then
        $compose_cmd exec -T mongo mongorestore \
            ...
            --authenticationDatabase admin \
            --db "${DB_NAME:-worklenz_db}" \
            --archive < "$temp_dir/$backup_content/database.archive" 2>/dev/null
...
configure_env() {
    ...
    # Passwords
    echo ""
    read -sp "Enter MongoDB password (or press Enter to generate): " db_password
    echo
