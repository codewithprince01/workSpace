...
const api = createApi({
  ...
  endpoints: builder => ({
    ...
    getProjects: builder.query<IServerResponse<IProject[]>, { view: number; teamId?: string }>({
      query: ({ view, teamId }) => {
        const timestamp = new Date().getTime();
        const teamParam = teamId ? `&team_id=${teamId}` : '';
        return `${rootUrl}/projects?view=${view}${teamParam}&t=${timestamp}`;
      },
      providesTags: (result, error, arg) => [{ type: 'teamProjects', id: arg.teamId || 'default' }],
