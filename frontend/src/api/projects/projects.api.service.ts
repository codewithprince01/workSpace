...
export const projectsApiService = {
  getProjects: async (
    ...
    const queryParams: any = { index, size, field, order, search: s, statuses, categories };
    
    if (filter === 1) queryParams.starred = true;
    else if (filter === 2) queryParams.archived = true;
    
    const url = `${rootUrl}${toQueryString(queryParams)}`;
  ...
  getGroupedProjects: async (
    ...
    const s = encodeURIComponent(search || '');
    const queryParams: any = { index, size, field, order, search: s, groupBy, statuses, categories, t: new Date().getTime() };
    
    if (filter === 1) queryParams.starred = true;
    else if (filter === 2) queryParams.archived = true;
    
    const url = `${rootUrl}/grouped${toQueryString(queryParams)}`;
  ...
  toggleFavoriteProject: async (id: string): Promise<IServerResponse<IProjectsViewModel>> => {
    ...
    const response = await apiClient.put<IServerResponse<IProjectsViewModel>>(`${url}`);
    return response.data;
  },
  ...
  inviteMember: async (
    ...
    const url = `${rootUrl}/${projectId}/invite`;
    const response = await apiClient.post<IServerResponse<any>>(url, { email, role });
    return response.data;
  },
  
  getUserRole: async (projectId: string): Promise<IServerResponse<{ role: string; project_id: string }>> => {
    const url = `${rootUrl}/${projectId}/role`;
    const response = await apiClient.get<IServerResponse<{ role: string; project_id: string }>>(url);
    return response.data;
  },
};
