...
const notificationSlice = createSlice({
  ...
  extraReducers: builder => {
    ...
    builder.addCase(fetchInvitations.fulfilled, (state, action) => {
      state.loading = false;
      const invitationsArray = Array.isArray(action.payload) ? action.payload : [];
      state.invitations = invitationsArray;
      state.invitationsCount = invitationsArray.length;
    ...
    builder.addCase(fetchNotifications.fulfilled, (state, action) => {
      ...
      const payload = action.payload as IWorklenzNotification[] | { data: IWorklenzNotification[] };
      const notificationsArray: IWorklenzNotification[] = Array.isArray(payload) 
        ? payload 
        : ((payload as { data: IWorklenzNotification[] })?.data || []);
      
      state.notifications = notificationsArray;
      state.notificationsCount = notificationsArray.length;

      notificationsArray.forEach((notification: IWorklenzNotification) => {
        state._dataset.push({
          type: 'notification',
          data: notification,
        });
