...
const taskSlice = createSlice({
  ...
  reducers: {
    ...
    updateTaskLoggedTime: (
      ...
      const { taskId, addedHours } = action.payload;
      const taskInfo = findTaskInGroups(state.taskGroups, taskId);
      if (taskInfo) {
        const task = taskInfo.task as any;
        const current =
          typeof task.total_logged_time === 'number'
            ? task.total_logged_time
            : parseFloat(task.total_logged_time || '0');
        task.total_logged_time = current + addedHours;

        if (task.timeTracking) {
          const currentTotal = parseFloat(task.timeTracking.total || '0');
          const newTotal = currentTotal + addedHours;
          task.timeTracking.total = newTotal.toFixed(2);
          task.timeTracking.logged = newTotal.toFixed(2);
        }
      }
    },
    ...
    addCustomColumn: (state, action: PayloadAction<ITaskListColumn>) => {
      const normalizedColumn = {
        ...action.payload,
        key: action.payload?.key || action.payload?.id,
        name: action.payload?.name || 'Text',
        pinned: true,
      };
    ...
    updateCustomColumnPinned: (
      ...
      const customColumn = state.customColumns.find(
        col => col.id === columnId || col.key === columnId
      );
      const column = state.columns.find(col => col.id === columnId || col.key === columnId);
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(fetchTaskGroups.fulfilled, (state, action) => {
        ...
        // Sync active timers from fetched tasks
        if (state.allTasks) {
          state.allTasks.forEach((t: any) => {
            if (t.timeTracking?.activeTimer) {
              state.activeTimers[t.id] = t.timeTracking.activeTimer;
            }
          });
        }
      })
      ...
      .addCase(fetchSubTasks.fulfilled, (state, action) => {
        if (action.payload && action.payload.groups && action.payload.groups.length > 0) {
          ...
          // Sync active timers for subtasks
