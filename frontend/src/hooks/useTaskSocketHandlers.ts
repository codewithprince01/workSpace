...
export const useTaskSocketHandlers = () => {
  ...
  const handlePriorityChange = useCallback(
    (response: ITaskListPriorityChangeResponse | string) => {
      console.log('🎯 useTaskSocketHandlers handlePriorityChange received:', response);
      if (!response) return;

      // Handle both string and object response formats
      const data: ITaskListPriorityChangeResponse = typeof response === 'string' 
        ? JSON.parse(response) 
        : response;
      ...
      if (currentTask) {
        ...
        // priority_id from backend is now the priority name (low, medium, high, urgent)
        let newPriorityValue = data.priority_id || 'medium';

        // Check if priority_id matches a priority in the list
        const priority = priorityList.find(p => p.id === data.priority_id || p.name?.toLowerCase() === data.priority_id?.toLowerCase());
        if (priority?.name) {
          newPriorityValue = priority.name.toLowerCase();
        }
        ...
        if (groups && groups.length > 0 && currentGrouping === 'priority') {
          // Find current group containing the task
          const currentGroup = groups.find(group => group.taskIds.includes(data.id));
          ...
          } else if (!targetGroup && data.priority_id) {
