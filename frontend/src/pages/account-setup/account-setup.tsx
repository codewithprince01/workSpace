...
const AccountSetup: React.FC = () => {
  ...
  const { t, i18n, ready } = useTranslation('account-setup', { useSuspense: false });
  useDocumentTitle('Account Setup'); // Static title to avoid i18n issues
  ...
  useEffect(() => {
    // Prevent setup page access if already completed
    ...
    const loadSurvey = async () => {
      try {
        ...
        console.log('Survey loading failed, continuing without survey');
      }
    };
  ...
  }, [ready, dispatch]); // Run only when i18n is ready
  ...
  const completeAccountSetup = async (skip = false) => {
    try {
      ...
      console.log('Setup API completed:', res);
      
      if (res.done && res.body.id) {
        trackMixpanelEvent(skip ? evt_account_setup_skip_invite : evt_account_setup_complete);
        
        // Refresh user session to update setup_completed status BEFORE redirect
        try {
          ...
          if (authResponse?.authenticated && authResponse?.user) {
            ...
            console.log('Session updated with setup_completed:', authResponse.user.setup_completed);
          }
        } catch (error) {
  ...
  const saveSurveyData = async () => {
    if (!surveyId || !surveyData) {
      console.log('No survey ID or data, skipping survey save');
      return;
    }
    ...
    try {
      ...
      if (!surveyResponse.done || !surveyResponse.body?.questions) {
        console.log('No survey questions available, skipping survey save');
        return;
      }
      ...
      if (answers.length > 0) {
        ...
        if (result.done) {
          ...
          console.log('Survey submission returned unsuccessful response');
        }
      } else {
        console.log('No survey answers to save');
      }
    } catch (error) {
      console.log('Failed to save survey data, continuing setup');
  ...
  return (
    ...
    >
      ...
      >
        {ready ? t('setupYourAccount', 'Account Setup') : 'Account Setup'}
      </Title>
          ...
          <div className="w-full max-w-2xl mt-8">
            <div className={`flex ${
              ...
              <Button
                ...
                onClick={nextStep}
              >
                {ready ? t('continue', 'Continue') : 'Continue'}
              </Button>
            </div>
          </div>
        </div>
