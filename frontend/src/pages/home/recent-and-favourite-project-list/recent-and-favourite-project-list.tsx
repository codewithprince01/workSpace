...
const RecentAndFavouriteProjectList = () => {
  ...
  // Get current team_id from Redux (updates immediately on team switch)
  ...
  const {
    ...
    view: getActiveProjectsFilter(),
    teamId: activeTeamId || undefined // Pass team_id to force cache invalidation on team switch
  });

  // Refetch data when team changes
  useEffect(() => {
    if (activeTeamId) {
      console.log('🔄 [PROJECTS-LIST] Team changed, refetching projects:', activeTeamId);
      refetch();
    }
      ...
      },
      {
        ...
        render: (record: IProjectViewModel) => (
          <Typography.Paragraph
            ...
            onClick={() =>
              navigate(`/workspace/projects/${record.id}?tab=tasks-list&pinned_tab=tasks-list`)
            }
          >
