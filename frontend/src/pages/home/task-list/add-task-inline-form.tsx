...
const AddTaskInlineForm = ({ t, calendarView }: AddTaskInlineFormProps) => {
  ...
  const handleTaskSubmit = (values: { name: string; project: string; dueDate: string }) => {
    ...
    if (!values.name?.trim() || !values.project) {
      console.warn('Task name or project is missing');
      return;
    }
    ...
    // Use 'once' to avoid memory leaks from adding multiple listeners
    socket?.once(SocketEvents.QUICK_TASK.toString(), (task: IMyTask) => {
      ...
      if (task && task.id) {
        ...
        socket?.once(
          ...
          (response: ITaskAssigneesUpdateResponse) => {
            console.log('📥 Assignees updated:', response);
            refetch();
          }
        );
      } else {
        // Still refetch even if assignee update is not needed
        refetch();
      }
    });
  ...
  return (
    ...
    >
      <Flex vertical gap={4} style={{ width: '100%', maxWidth: 400 }}>
        <Form.Item
          name="name"
