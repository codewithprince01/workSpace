...
const TaskTimer = ({
  ...
  const getDisplayTime = () => {
    if (started) return timeString;
    
    if (!totalLoggedTime) return '0h 0m 0s';
    
    // Assume totalLoggedTime is in hours (string "5.25" or number 5.25)
    // Convert to seconds for formatTimeSpent (which expects seconds)
    const hours = typeof totalLoggedTime === 'string' ? parseFloat(totalLoggedTime) : totalLoggedTime;
    
    if (isNaN(hours) || hours === 0) return '0h 0m 0s';
    
    return formatTimeSpent(Math.round(hours * 3600));
  };
  ...
  return (
    <Flex gap={4} align="center">
      ...
      >
        <Typography.Text style={{ cursor: 'pointer' }}>{getDisplayTime()}</Typography.Text>
      </Popover>
    </Flex>
  );
};
