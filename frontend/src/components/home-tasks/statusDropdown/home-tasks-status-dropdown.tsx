...
const HomeTasksStatusDropdown = ({ task, teamId }: HomeTasksStatusDropdownProps) => {
  ...
  const handleStatusChange = (statusId: string) => {
    console.log('🔄 handleStatusChange called:', { taskId: task.id, statusId, socket: !!socket, connected });
    if (!task.id || !statusId) {
      console.log('❌ Missing task.id or statusId');
      return;
    }
    ...
    console.log('📤 Emitting TASK_STATUS_CHANGE:', payload);
    socket?.emit(
