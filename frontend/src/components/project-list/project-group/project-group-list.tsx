...
const ProjectGroupList: React.FC<ProjectGroupListProps> = ({
  ...
  const [toggleFavorite] = useToggleFavoriteProjectMutation();
  ...
  const handleFavoriteClick = async (
    ...
    e.stopPropagation();
    try {
      await toggleFavorite(projectId).unwrap();
      ...
      <Col key={project.id} xs={24} sm={12} md={8} lg={6} xl={4}>
        ...
        >
          ...
          <div className="action-buttons" style={styles.actionButtons}>
            <Tooltip title={project.is_favorite ? t('removeFromFavorites') : t('addToFavorites')}>
              <button
